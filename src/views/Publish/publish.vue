<template>
  <div id="publish">
    <v-header title="发布文章" :back="true" :button="'icon-send'" @clickButton="publish"></v-header>
  ...
  export default {
    ...
    methods: {
      selectFileChange (e) {
        ...
        if (this.images.length >= 6) {
          this.$Toast({iConClass: 'icon-false', waiting: false, tip: '最多上传6张图片'})
          return false
        }
      ...
      getImages (files) {
        ...
        if (typeof FileReader === 'undefined') {
          this.$Toast({iConClass: 'icon-false', waiting: false, tip: '您的浏览器无法上传'})
          return false
        }
        if (files[0].size > 1048576) {
          this.$Toast({iConClass: 'icon-false', waiting: false, tip: '图片大于1M'})
          return false
        }
        const _this = this
      ...
      publish () {
        if (this.title === '') {
          this.$Toast({iConClass: 'icon-false', waiting: false, tip: '请填写标题'})
          return
        }
        if (this.content === '') {
          this.$Toast({iConClass: 'icon-false', waiting: false, tip: '请填写内容'})
          return
        }
        this.$Popup({title: '提示', content: '确定发布此篇文章？', confirmButton: '发布'}).then(response => {
          this.uploadImages(this.images).then(response => {
            ...
            this.$HideToast()
            this.$Toast({iConClass: 'icon-success', waiting: false, tip: '发布成功'})
            this.$router.back()
          }).catch(response => {
            this.$Toast({iConClass: 'icon-false', waiting: false, tip: response.data ? response.data.message : '发布失败'})
          })
        }).catch(response => {})
      }
    },
